...
public class FallingPlatforms : MonoBehaviour
{
    ...
    private float idleAmplitudeY = 0.2f; //위아래로 떠다니는 정도
    private float idleAmplitudeX = 0.1f; //좌우로 떠다니는 정도
    private float idleSpeed = 3f;      //기본적으로 움직이는 속도
    private float fallDelay = 1.0f;    //밞은 후 떨어지기까지의 시간
    ...
    private Vector3 originPos; //기본 위치
    private bool isStepped = false; //멈춤 상태
    ...
    void Update() {
        ...
        if (!isStepped) {
            ...
            float newY = originPos.y + Mathf.Sin(currentPos) * idleAmplitudeY;
            
            //좌우 이동은 코사인파 이용
            float newX = originPos.x + Mathf.Cos(currentPos * 0.8f) * idleAmplitudeX;
            
            //y좌표를 sin을 적용해서 새로 만든 y좌표로 변경
            transform.position = new Vector3(newX, newY, originPos.z);
        }
    }
    ...
    private void OnCollisionEnter2D(Collision2D collision) {
        ...
        if (collision.gameObject.CompareTag("Player") && !isStepped) {
            ...
            if (normal.y < -0.5f)
            {
                //파티클 종료, 애니메이션 실행, 중복 밟힘 방지, 떨어지기 시작
                FlyDust.Stop();
